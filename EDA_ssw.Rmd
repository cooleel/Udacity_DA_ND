...
corrplot(cor(wine) , method = "number", type="lower",diag=FALSE )
```
...
wine$overall <- factor(ifelse(wine$quality<=5,"poor","good"),levels = c("poor","good"))
```
...
####Alcohol of wines in differnt qualities
...
p1 <- ggplot(data = wine,aes(x=alcohol,y=volatile.acidity))+
  geom_point(alpha=1/4,aes(color=overall))

p2 <- ggplot(data = wine, aes(x=sulphates,y=volatile.acidity))+
  xlim(0.25,quantile(wine$sulphates,0.99))+
  geom_point(alpha=1/4,aes(color=overall))

p3 <- ggplot(data = wine,aes(x=alcohol,y=sulphates))+
  geom_point(alpha=1/4,aes(color=overall))

p4<- ggplot(data = wine,aes(x=sulphates,y=log(total.sulfur.dioxide)))+
  geom_point(alpha=1/4,aes(color=overall))

p5<- ggplot(data = wine, aes(x=alcohol,y=log(total.sulfur.dioxide)))+
  geom_point(alpha=1/4,aes(color=overall))

p6<- ggplot(data = wine, aes(x= volatile.acidity,y=log(total.sulfur.dioxide)))+
  geom_point(alpha=1/4,aes(color=overall))
...
p1 <- ggplot(data = wine,aes(x=rating,y=alcohol))+
  geom_point(alpha=1/4)+annotate("text",label="Mean", x= 6.35,y= 12.2,size=4,color ='red')+
  stat_summary(aes(y=alcohol,group=1),fun.y=mean,color='red',geom='line',group=1)

p2 <-ggplot(data = wine,aes(x=rating,y=sulphates))+
  geom_point(alpha=1/4)+annotate("text",label="Mean", x= 6.35,y= 0.8,size=4,color ='red')+
  ylim(0.25,quantile(wine$sulphates,0.99))+
  stat_summary(aes(y=sulphates,group=1),fun.y=mean,color='red',geom='line',group=1)

p3 <- ggplot(data = wine,aes(x=rating,y=volatile.acidity))+
  geom_point(alpha=1/4)+annotate("text",label="Mean", x= 6.35,y= 0.4,size=4,color ='red')+
  stat_summary(aes(y=volatile.acidity,group=1),fun.y = mean,color='red',geom = "line",group=1)

p4 <- ggplot(data = wine,aes(x=rating,y=pH))+
  geom_point(alpha=1/4)+annotate("text",label="Mean", x= 6.35,y= 3.2,size=4,color ='red')+
  stat_summary(aes(y=pH,group=1),fun.y = mean,color='red',geom = "line",group=1)
...
#generate model
model <- glm(overall~.,family = binomial(link = "logit"),data=train[,c(1:11,14)])
summary(model)
```
...
### Plot Two
```{r echo=FALSE, Plot_One,warning=FALSE,message=FALSE,fig.width=12,fig.height=9}
p1 <- ggplot(data = wine,aes(x=alcohol,y=volatile.acidity))+
  geom_point(alpha=1/4,aes(color=overall))+
  xlab("Alcohol(% by volume)")+
  ylab("Volatile.acidity(g/dm^3)")
...
p4<- ggplot(data = wine,aes(x=sulphates,y=log(total.sulfur.dioxide)))+
  geom_point(alpha=1/4,aes(color=overall))+
  xlab("Sulphates(g/dm^3)")+
  ylab("log(Total.sulfur.dioxide(mg/dm^3))")
